...
<h2>Request A Seasonal Budget</h2>
<p>Plan, team up and request a budget for the upcoming season on Protein DAO.</p>

{#if okrs === false}
  <div class="okr-report">
    Please define your team's OKRs in Notion before requesting a budget.
  </div>
{:else if okrs === true}
  <Loading />
{:else if okrs.length == 0}
  <div class="okr-report">
    There is no OKRs defined for this season.
  </div>
{:else}
  <div class="okr-report">
    Community has the following OKRs defined for the <strong>{nextSeasonTitle}</strong>:
    <br><br>
    <div class="okr-report-content">
      <ol>
        {#each okrs as objective}
          <li><strong>{objective.category + ': ' + objective.description}</strong>
            <ul>
